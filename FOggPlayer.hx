...
class FOggPlayer
{
	...
	titleText.y = (playButton.height - titleText.height) / 2;
	volumeControl.x = titleText.x + titleText.width + 3;
	volumeControl.y = titleText.y;
	...
	// Set the volume, if changed

	if (volume != 1.0)
	    volumeControl.set(volume);

	// Start if play is true

	if (play == true)
	    playSound(null);

	// Set up external interface if enabled

	if ((external == true) && (ExternalInterface.available == true))
	{
	    try
	    {
		ExternalInterface.addCallback("Start", playSound);
		ExternalInterface.addCallback("start", playSound);
		ExternalInterface.addCallback("Cease", stopSound);
		ExternalInterface.addCallback("cease", stopSound);
	    ...
	    {
		titleText.text = e.name + ": " + e.errorID;
		restoreTitle();
	    }
	}
    }
	...
	var width = titleText.width + volumeControl.width + 10;
	var height = titleText.height + 6;

	matrix.createGradientBox(width - 2, height - 2, Math.PI/2, 0, 0);
	...
	g.drawRoundRect(titleText.x - 4, 0, width, height, 5, 5);
	g.endFill();

	// Change the colours for the text field background, and set
	// up another gradient to look convex
    ...
    {
	var clip = Lib.current;
	var pars = clip.loaderInfo.parameters;
	...
	// See if there's a play parameter

	if (pars.play != null)
	{
	    if (pars.play.toLowerCase() == "true")
		play = true;
	}

	// See if there's a start parameter

	if (pars.start != null)
	{
	    if (pars.start.toLowerCase() == "true")
		play = true;
	}

	// See if there's a volume parameter

	if (pars.volume != null)
	{
	    volume = Std.parseInt(pars.volume) / 100.0;
	...
	// See if there's an external parameter

	if (pars.external != null)
	{
	    if (pars.external.toLowerCase() == "true")
		external = true;
	}
	...
	if (url != null)
	{
		...
		if (channel != null)
		{
		    // Set the volume, don't understand why you have to
		    // get the transform, change it and put it back, but
	...
	if (channel != null)
	{
	    ...
	    var mins = Math.floor(position / 60000);
	    var secs = Math.round((position - (mins * 60000)) / 10) / 100;
	    var perc = Math.round((position / sound.length) * 100);

	    titleText.text = "Pause: " + mins + " mins  " +
		secs + " secs  " + perc + "%";
	...
	var secs = Math.round((sound.length - (mins * 60000)) / 10) / 100;
